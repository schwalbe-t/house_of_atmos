...
namespace houseofatmos::world {
    ...
    void Research::report_item_production(Item::Type item, u64 count) {
    ...
    void Research::check_completion(Toasts& toasts) {
        for(auto& [advancement, progress]: this->progress) {
            ...
            for(Advancement parent: info.parents) {
                progress.is_unlocked &= this->is_unlocked(parent);
            }
            size_t item_c_c = progress.item_conditions.size();
            for(size_t item_c_i = 0; item_c_i < item_c_c; item_c_i += 1) {
    ...
    bool Research::is_unlocked(Advancement advancement) const {
