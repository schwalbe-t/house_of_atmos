...
struct ModelShader: engine::rendering::Shader<engine::resources::ModelVertex, ModelShader> {
    ...
    Vec<4> vertex(engine::resources::ModelVertex vertex) override {
        this->pos = (this->model * vertex.pos.with(1.0)).swizzle<3>("xyz");
        this->uv = vertex.uv;
        this->normal = (this->model * vertex.normal.with(1.0)).swizzle<3>("xyz");
    ...
    Vec<4> fragment() override {
        this->interpolate(&this->pos);
        this->interpolate(&this->uv);
        this->interpolate(&this->normal);
        double reflected = this->normal.dot((this->light - this->pos).normalized());
        return this->tex->sample(this->uv)
            * (std::min(std::max(reflected, 0.0), 1.0) * (1.0 - this->ambient) + ambient);
    }
};
...
int main() {
    ...
    auto sub_buffer = engine::rendering::Surface(400, 300);
